...
func (f *renewalFakeStore) CreateNonce(ctx context.Context, nonce string, expiresAt time.Time) error {
	return nil
}
func (f *renewalFakeStore) ConsumeNonce(ctx context.Context, nonce string) (bool, error) {
	return true, nil
}
...
func TestHandler_RenewalInfo_GET(t *testing.T) {
	...
	svc := internalacme.NewService(store, nil, internalacme.NewNonceManager(store, 0), nil, nil, "https://ca.test")
...
func TestService_RenewalInfoURLShape(t *testing.T) {
	svc := internalacme.NewService(&renewalFakeStore{certs: map[uuid.UUID]*storage.Certificate{}, provs: map[uuid.UUID]*storage.Provisioner{}}, nil, internalacme.NewNonceManager(nil, 0), nil, nil, "https://ca.test")
	provID := uuid.New()
...
func TestHandler_Directory_AdvertisesRenewalInfo(t *testing.T) {
	store := &renewalFakeStore{
		certs: map[uuid.UUID]*storage.Certificate{},
		provs: map[uuid.UUID]*storage.Provisioner{},
	}
	...
	svc := internalacme.NewService(store, nil, internalacme.NewNonceManager(store, 0), nil, nil, "https://ca.test")
	h := NewACMEHandler(store, nil, svc, config.ACMEConfig{BaseURL: "https://ca.test"}, nil)

	r := chi.NewRouter()
	h.RegisterRoutes(r)

	req := httptest.NewRequest(http.MethodGet, "/acme/"+provID.String()+"/directory", nil)
	rec := httptest.NewRecorder()
	r.ServeHTTP(rec, req)
	...
	want := "https://ca.test/acme/" + provID.String() + "/renewal-info/"
